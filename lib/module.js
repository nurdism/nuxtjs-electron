...
const config = (options, nuxt, generate) => {
  return {
    mode: generate ? 'production' : 'development',

    entry: {
      main: path.join(nuxt.srcDir, options.entry)
    },
    ...
    output: {
      ...
      path: generate ? path.join(nuxt.rootDir, nuxt.generate.dir) : nuxt.buildDir
    },
...
const start = (config, options, nuxt) => {
  ...
  instance = spawn(electron, [path.join(config.output.path, 'main.js')], { env: process.env })
